package leetcode.DP.TwoDimension;

/**
 *
 * Given a m * n matrix of ones and zeros, return how many square submatrices have all ones.
 *
 *
 *
 * Example 1:
 *
 * Input: matrix =
 * [
 *   [0,1,1,1],
 *   [1,1,1,1],
 *   [0,1,1,1]
 * ]
 * Output: 15
 * Explanation:
 * There are 10 squares of side 1.
 * There are 4 squares of side 2.
 * There is  1 square of side 3.
 * Total number of squares = 10 + 4 + 1 = 15.
 * Example 2:
 *
 * Input: matrix =
 * [
 *   [1,0,1],
 *   [1,1,0],
 *   [1,1,0]
 * ]
 * Output: 7
 * Explanation:
 * There are 6 squares of side 1.
 * There is 1 square of side 2.
 * Total number of squares = 6 + 1 = 7.
 *
 *
 * Constraints:
 *
 * 1 <= arr.length <= 300
 * 1 <= arr[0].length <= 300
 * 0 <= arr[i][j] <= 1
 *
 *
 */

public class _1277_CountSquareSubmatricesWithAllOnes {
    //=============================================================================================
    //Top Down Approach
    public int countSquares(int[][] matrix) {
        int ans = 0;
        int memo[][] = new int[matrix.length][matrix[0].length];

        for (int r = 0; r < matrix.length; r++) {
            for (int c = 0; c < matrix[0].length; c++) {
                ans += recur(matrix, r, c, memo);
            }
        }
        return ans;
    }

    private int recur(int[][] matrix, int r, int c, int[][] memo) {

        if (r < 0 || c < 0 || r == matrix.length || c == matrix[0].length ||
                matrix[r][c] == 0) {
            return 0;
        }

        if (memo[r][c] > 0) {
            return memo[r][c];
        }
        int right = recur(matrix, r + 1, c, memo);
        int down = recur(matrix, r, c + 1, memo);
        int diagonal = recur(matrix, r + 1, c + 1, memo);
        return memo[r][c] = 1 + Math.min(diagonal, Math.min(right, down));
    }
    //=============================================================================================
    //Bottom up approach
    public int countSquares1(int[][] matrix) {
        int ans = 0;
        int memo[][] = new int[matrix.length + 1][matrix[0].length + 1];

        for (int r = matrix.length - 1; r >= 0; r--) {
            for (int c = matrix[0].length - 1; c >= 0; c--) {

                if (matrix[r][c] != 0) {
                    int right = memo[r + 1][c];
                    int down = memo[r][c + 1];
                    int diagonal = memo[r + 1][c + 1];
                    memo[r][c] = 1 + Math.min(diagonal, Math.min(right, down));
                }
                ans += memo[r][c];
            }
        }
        return ans;
    }
}
