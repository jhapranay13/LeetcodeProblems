...
public class _1130_MinimumCostTreeFromLeafValues {
	...
	public int mctFromLeafValues1(int[] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			memo[i][i] = new Integer[] { arr[i], 0 };
		}

		for (int h = 1; h <= arr.length; h++) {
			for (int lo = 0; lo <= arr.length - h; lo++) {
				int hi = lo + h - 1;
				...
				for (int i = lo; i < hi; i++) {
					Integer[] left = memo[lo][i];
					Integer[] right = memo[i + 1][hi];
					int tempNodeVal = left[0] * right[0];
					int tempSum = left[1] + right[1] + tempNodeVal;
				...
				memo[lo][hi] = new Integer[] { nodeVal, sum };
			}
		}
		Integer ans = memo[0][arr.length - 1][1];
		return ans;
	}
}
