...
public class _1236_Web_Crawler {
    ...
    public List<String> crawl(String startUrl, HtmlParser htmlParser) {
        ...
        recur(startUrl, htmlParser, holder, splitUrl[2]);
        return new ArrayList<>(holder);
    }

    private void recur(String url, HtmlParser htmlParser, Set<String> holder, String host) {
        holder.add(url);

        for (String nextUrl : htmlParser.getUrls(url)) {

            if (nextUrl.split("/")[2].equals(host) && !holder.contains(nextUrl)) {
                recur(nextUrl, htmlParser, holder, host);
            }
        }
    }
}
