 ...
 * (of the same length) if in the first position where a and b differ, subsequence a has a number
 * less than the corresponding number in b. For example, [1,3,4] is more competitive than [1,3,5]
...
public class _1673_FindMostCompetitveSubsequence {

    public int[] mostCompetitive(int[] nums, int k) {
        Deque<Integer> mono = new LinkedList<>();
        int[] ans = new int[k];
        int numberToDiscard = nums.length - k;

        for (int i = 0; i < nums.length; i++) {

            while (!mono.isEmpty() && nums[mono.peekLast()] > nums[i] && numberToDiscard > 0) {
                numberToDiscard--;
                mono.pollLast();
            }
            mono.offerLast(i);
