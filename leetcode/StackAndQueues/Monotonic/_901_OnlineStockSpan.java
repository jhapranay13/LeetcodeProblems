 ...
 *         80, 60, 70, 60, 75, 85], then the stock spans would be [1, 1, 1, 2,
 *         1, 4, 6].
 * 
 * 
 * 
 ...
 *         Explanation: First, S = StockSpanner() is initialized. Then:
 *         S.next(100) is called and returns 1, S.next(80) is called and returns
 *         1, S.next(60) is called and returns 1, S.next(70) is called and
 *         returns 2, S.next(60) is called and returns 1, S.next(75) is called
 *         and returns 4, S.next(85) is called and returns 6.
 * 
 ...
 *         test cases. The total time limit for this problem has been reduced by
 *         75% for C++, and 50% for all other languages.
 *
 */
...
public class _901_OnlineStockSpan {
	...
	class StockSpanner1 {
		...
		public int next(int price) {
			...
			if (mono.isEmpty()) {
				mono.offer(new int[] {price, index++});
				return 1;
			}

			while (!mono.isEmpty() && mono.peekLast()[0] <= price) {
				mono.pollLast();
			}
			int ans = mono.isEmpty() ? index + 1 : index - mono.peekLast()[1];
			mono.offer(new int[] {price, index++});
			return ans;
		}
	}
