 ...
 *         5 and 6 respectively. All other sub-arrays of size 3 have averages
 ...
 *         Output: 6 Explanation: The first 6 sub-arrays of size 3 have averages
 ...
 *         Constraints:
 * 
 *         1 <= arr.length <= 10^5 1 <= arr[i] <= 10^4 1 <= k <= arr.length 0 <=
 *         threshold <= 10^4
 *
 */

public class _1343_NumSubArraysAvargeSizeKLessOrEqualToThreshold {
	...
	public int numOfSubarrays(int[] arr, int k, int threshold) {
		...
		while (fast < arr.length) {
			...
			if ((fast - slow + 1) == k && (total / k) >= threshold) {
				ans++;
			}
			fast++;
		}
