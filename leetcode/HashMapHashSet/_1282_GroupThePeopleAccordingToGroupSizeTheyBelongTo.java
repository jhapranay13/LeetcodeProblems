 ...
 *         Explanation: The first group is [5]. The size is 1, and groupSizes[5]
 *         = 1. The second group is [0,1,2]. The size is 3, and groupSizes[0] =
 *         groupSizes[1] = groupSizes[2] = 3. The third group is [3,4,6]. The
 *         size is 3, and groupSizes[3] = groupSizes[4] = groupSizes[6] = 3.
...
public class _1282_GroupThePeopleAccordingToGroupSizeTheyBelongTo {
	...
	public List<List<Integer>> groupThePeople(int[] groupSizes) {
		...
		for (int i = 0; i < groupSizes.length; i++) {
			List<Integer> list = groupIndexMap.getOrDefault(groupSizes[i], new ArrayList<>());
			list.add(i);
			groupIndexMap.put(groupSizes[i], list);
		}
