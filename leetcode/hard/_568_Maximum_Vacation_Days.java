...
public class _568_Maximum_Vacation_Days {
    public int maxVacationDays(int[][] flights, int[][] days) {
        ...
        return recur(flights, days, 0, 0, memo);
    }

    private int recur(int[][] flights, int[][] days, int currCity, int currWeek, Integer[][] memo) {

        if (currWeek == days[0].length) {
            return 0;
        }

        if (memo[currCity][currWeek] != null) {
            return memo[currCity][currWeek];
        }
        // Stay in current city in this week
        int ans = days[currCity][currWeek] + recur(flights, days, currCity, currWeek + 1, memo);

        for (int i = 0; i < flights.length; i++) {

            if (flights[currCity][i] == 1) {
                int temp = days[i][currWeek] + recur(flights, days, i, currWeek + 1, memo);
                ans = Math.max(ans, temp);
            }
        }
        return memo[currCity][currWeek] = ans;
    }
}
