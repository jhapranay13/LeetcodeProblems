        ...
        while (!pq.isEmpty()) {
            ...
            for (int i = 0; i < dirs.length; i++) {
                ...
                if (matrix.containsKey(key)) {
                    int val = matrix.remove(key);
                    pq.offer(new int[] {nr, nc, val + curr[2]});
                }
            }
        }
        return -1;
    }
    private char[] direction = {'U', 'R', 'D', 'L'};
    private int dirs[][] = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
    int[] target = null;

    private void  recur(GridMaster master, Map<String, Integer> matrix, int r, int c) {
        ...
        for (int i = 0; i < dirs.length; i++) {
            ...
            if (!matrix.containsKey(key) && master.canMove(direction[i])) {
                int val = master.move(direction[i]);
                matrix.put(key, val);
                recur(master, matrix, nr, nc);
                master.move(direction[(i + 2) % 4]); //reverse movement
            }
        }
    }
}
