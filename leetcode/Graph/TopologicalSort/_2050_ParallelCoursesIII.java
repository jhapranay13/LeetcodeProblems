...
public class _2050_ParallelCoursesIII {
    ...
    public int minimumTime(int n, int[][] relations, int[] time) {
        ...
        while (!q.isEmpty()) {
            int[] courseTime = q.poll();
            incoming[courseTime[0]]--;
            processedCourseCount++;

            if (processedCourseCount == n) {
                return courseTime[1];
            }
            List<Integer> adj = adjMap.getOrDefault(courseTime[0], new ArrayList<>());

            for (int course : adj) {
                incoming[course]--;

                if (incoming[course] == 0) {
                    q.offer(new int[] {course, courseTime[1] + time[course - 1]});
                }
            }
        }
        return -1;
