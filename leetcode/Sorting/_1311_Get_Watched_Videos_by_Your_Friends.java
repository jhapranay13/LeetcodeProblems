...
public class _1311_Get_Watched_Videos_by_Your_Friends {
    public List<String> watchedVideosByFriends(List<List<String>> watchedVideos, int[][] friends, int id, int level) {
        ...
        while (!q.isEmpty()) {
            ...
            while (size-- > 0) {
                int person = q.poll();
                holder.add(person);
                int[] friend = friends[person];

                for (int frnds : friend) {
        ...
        for (String key : videoFreq.keySet()) {
            ans.add(key);
        }
