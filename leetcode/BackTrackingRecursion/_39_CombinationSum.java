...
public class _39_CombinationSum {
	...
	public List<List<Integer>> combinationSum(int[] candidates, int target) {
		...
		recur(candidates, ans, partial, target, 0, 0);
		return ans;
	}

	private void recur(int[] candidates, List<List<Integer>> ans, 
			ArrayList<Integer> partial, int target, int sum, int index) {
		...
		for (int i = index; i < candidates.length; i++) {
			...
			recur(candidates, ans, partial, target, sum + candidates[i], i);
