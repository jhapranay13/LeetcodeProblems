...
public class _2337_Move_Pieces_to_Obtain_a_String {
    public boolean canChange(String start, String target) {
        ...
        while (sIndex < start.length() || tIndex < target.length()) {
            ...
            while (sIndex < start.length() && start.charAt(sIndex) == '_') {
                sIndex++;
            }

            while (tIndex < target.length() && target.charAt(tIndex) == '_') {
                tIndex++;
            }

            if (sIndex == start.length() || tIndex == target.length()) {
                return tIndex == target.length() && sIndex == start.length();
            }
