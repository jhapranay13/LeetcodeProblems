 ...
 *         integers using 2's complement notation. Therefore, in Example 2
 *         above, the input represents the signed integer -3 and the output
 *         represents the signed integer -1073741825. Follow up:
 * 
 ...
 *         (00111001011110000010100101000000) Explanation: The input binary
 *         string 00000010100101000001111010011100 represents the unsigned
 ...
 *         (10111111111111111111111111111111) Explanation: The input binary
 *         string 11111111111111111111111111111101 represents the unsigned
...
public class _190_ReverseBits {
    ...
    public int reverseBits(int n) {
        int res = 0,j=0;
        for(int i = 31; i>=0;i--){
            //chech whether bit is set at i position in n
            if((n & (1<<i))!=0){
                //set the bit in result also
                res |= 1<<j;
            }
            j++;
        }
    ...
    public int reverseBits1(int n) {
        ...
        while (lo < hi) {
            int bitAtLo = n & maskLo;
            int bitAtHi = n & maskHi;

            if ((bitAtLo != 0 && bitAtHi == 0) || bitAtLo == 0 && bitAtHi != 0) {
                n ^= maskLo;
                n ^= maskHi;
            }
            lo++;
