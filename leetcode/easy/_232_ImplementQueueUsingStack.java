 ...
 *         valid. Depending on your language, the stack may not be supported
 *         natively. You may simulate a stack using a list or deque
 *         (double-ended queue) as long as you use only a stack's standard
 *         operations. Follow-up: Can you implement the queue such that each
 ...
 *         Example 1:
 * 
 *         Input ["MyQueue", "push", "push", "peek", "pop", "empty"] [[], [1],
 *         [2], [], [], []] Output [null, null, null, 1, 1, false]
 * 
 *         Explanation MyQueue myQueue = new MyQueue(); myQueue.push(1); //
 *         queue is: [1] myQueue.push(2); // queue is: [1, 2] (leftmost is front
 *         of the queue) myQueue.peek(); // return 1 myQueue.pop(); // return 1,
 *         queue is [2] myQueue.empty(); // return false
 * 
 * 
 *         Constraints:
 * 
 *         1 <= x <= 9 At most 100 calls will be made to push, pop, peek, and
 *         empty. All the calls to pop and peek are valid.
 *
 */
/**
